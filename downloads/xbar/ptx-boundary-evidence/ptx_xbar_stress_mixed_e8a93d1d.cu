...
struct Options {
    ...
    int mixed_iterations = 2048;
};

enum MixedFlags : uint32_t {
    kMixedCompute = 1u << 0,
    kMixedL2 = 1u << 1,
    kMixedVram = 1u << 2,
    kMixedAtomic = 1u << 3,
...
__global__ void mixed_kernel(const uint32_t *l2_data, uint32_t l2_mask,
                             const uint32_t *vram_data, uint32_t vram_mask,
                             uint32_t *compute_output, uint64_t *bins,
                             uint64_t *errors, uint64_t *digest,
                             uint64_t *sm_hits, uint64_t *work_items,
                             int iterations, int target_sm, uint32_t flags) {
    uint32_t smid = read_smid();
    if (!selected_sm(target_sm, smid))
        return;
    ...
    uint32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t x = mix32(tid ^ 0xa5a5f00du);
    uint32_t l2_state = mix32(tid ^ 0x31415926u);
    uint32_t vram_state = mix32(tid ^ 0x27182818u);
    uint64_t local_errors = 0;
    uint64_t local_digest = 0;

    for (int i = 0; i < iterations; ++i) {
        if (flags & kMixedCompute) {
#pragma unroll
            for (int j = 0; j < 8; ++j) {
                asm volatile("mad.lo.u32 %0, %0, %1, %2;"
                             : "+r"(x)
        ...
        if (flags & kMixedL2) {
            l2_state = l2_state * 747796405u + 2891336453u;
            uint32_t idx = l2_state & l2_mask;
            uint32_t got = load_global_cg(l2_data + idx);
            uint32_t expected = mix32(idx ^ 0x13579bdfu);
            local_errors += got != expected;
            local_digest += static_cast<uint64_t>(got) * (i + 1u);
        }
        if (flags & kMixedVram) {
            vram_state = vram_state * 277803737u + 1013904223u;
            uint32_t idx = vram_state & vram_mask;
            uint32_t got = load_global_cg(vram_data + idx);
            uint32_t expected = mix32(idx ^ 0x2468ace0u);
            local_errors += got != expected;
            local_digest ^= static_cast<uint64_t>(got) *
                            (static_cast<uint32_t>(i) | 1u);
        }
        if ((flags & kMixedAtomic) && ((i & 15) == 0)) {
            uint32_t bin = (tid + static_cast<uint32_t>(i)) & 1023u;
            atomic_add_global_u64(bins + bin, 1);
        }
    }

    if (flags & kMixedCompute)
        compute_output[tid] = x;
    if (local_errors)
        atomic_add_global_u64(errors, local_errors);
    atomic_add_global_u64(digest, local_digest);
    if (threadIdx.x == 0)
        atomic_add_global_u64(sm_hits + smid, 1);
    if (tid == 0) {
        uint64_t loads_per_thread =
            static_cast<uint64_t>(iterations) *
            (((flags & kMixedL2) ? 1u : 0u) +
             ((flags & kMixedVram) ? 1u : 0u));
        atomic_add_global_u64(
            work_items,
            loads_per_thread * static_cast<uint64_t>(gridDim.x) * blockDim.x);
    }
}
...
uint32_t mixed_flags_for_mode(const std::string &mode) {
    if (mode == "mix-cl")
        return kMixedCompute | kMixedL2;
    if (mode == "mix-la")
        return kMixedL2 | kMixedAtomic;
    if (mode == "mix-cla")
        return kMixedCompute | kMixedL2 | kMixedAtomic;
    if (mode == "mix-lv")
        return kMixedL2 | kMixedVram;
    if (mode == "mix-clva")
        return kMixedCompute | kMixedL2 | kMixedVram | kMixedAtomic;
    return 0;
}

bool run_mixed(const Options &opt, int sm_count, int blocks, int threads,
               uint64_t *d_errors, uint64_t *d_digest,
               uint64_t *d_sm_hits, uint64_t *d_work_items,
               uint32_t flags) {
    const size_t count = static_cast<size_t>(blocks) * threads;
    size_t l2_words = round_down_power_of_two(
        opt.l2_mib * 1024ull * 1024ull / sizeof(uint32_t));
    size_t vram_words = round_down_power_of_two(
        opt.vram_mib * 1024ull * 1024ull / sizeof(uint32_t));
    uint32_t *d_l2 = nullptr, *d_vram = nullptr, *d_output = nullptr;
    uint64_t *d_bins = nullptr;

    if (flags & kMixedL2) {
        CUDA_OK(cudaMalloc(&d_l2, l2_words * sizeof(uint32_t)));
        init_pattern_kernel<<<blocks, threads>>>(d_l2, l2_words, 0x13579bdfu);
    }
    if (flags & kMixedVram) {
        CUDA_OK(cudaMalloc(&d_vram, vram_words * sizeof(uint32_t)));
        init_pattern_kernel<<<blocks, threads>>>(d_vram, vram_words,
                                                 0x2468ace0u);
    }
    if (flags & kMixedCompute)
        CUDA_OK(cudaMalloc(&d_output, count * sizeof(uint32_t)));
    if (flags & kMixedAtomic) {
        CUDA_OK(cudaMalloc(&d_bins, 1024 * sizeof(uint64_t)));
        CUDA_OK(cudaMemset(d_bins, 0, 1024 * sizeof(uint64_t)));
    }
    CUDA_OK(cudaDeviceSynchronize());
    clear_common(d_errors, d_digest, d_sm_hits, d_work_items, sm_count);
    ...
    uint64_t start = monotonic_ns();
    double elapsed = 0.0;
    const Affine32 transform = affine_pow(
        kLcgMul, kLcgAdd,
        static_cast<uint32_t>(opt.mixed_iterations * 8));

    do {
        mixed_kernel<<<blocks, threads>>>(
            d_l2, static_cast<uint32_t>(l2_words - 1), d_vram,
            static_cast<uint32_t>(vram_words - 1), d_output, d_bins, d_errors,
            d_digest, d_sm_hits, d_work_items, opt.mixed_iterations,
            opt.target_sm, flags);
        CUDA_OK(cudaGetLastError());
        CUDA_OK(cudaDeviceSynchronize());
        if ((flags & kMixedCompute) && opt.target_sm < 0) {
            CUDA_OK(cudaMemcpy(output.data(), d_output,
                               count * sizeof(uint32_t),
                               cudaMemcpyDeviceToHost));
            for (size_t tid = 0; tid < count; ++tid) {
                uint32_t seed =
                    mix32(static_cast<uint32_t>(tid) ^ 0xa5a5f00du);
                uint32_t expected = static_cast<uint32_t>(
                    transform.mul * seed + transform.add);
                compute_errors += output[tid] != expected;
            }
        }
        CUDA_OK(cudaMemcpy(&device_errors, d_errors, sizeof(device_errors),
                           cudaMemcpyDeviceToHost));
        ++rounds;
        elapsed = (monotonic_ns() - start) / 1e9;
    } while (elapsed < opt.seconds && device_errors == 0 &&
             compute_errors == 0);

    uint64_t atomic_errors = 0;
    uint64_t atomic_total = 0;
    if (flags & kMixedAtomic) {
        std::vector<uint64_t> bins(1024);
        CUDA_OK(cudaMemcpy(bins.data(), d_bins, 1024 * sizeof(uint64_t),
                           cudaMemcpyDeviceToHost));
        for (uint64_t value : bins)
            atomic_total += value;
        if (opt.target_sm < 0) {
            uint64_t atomics_per_thread =
                static_cast<uint64_t>((opt.mixed_iterations + 15) / 16);
            uint64_t expected = static_cast<uint64_t>(count) *
                                atomics_per_thread * rounds;
            atomic_errors = atomic_total != expected;
        } else {
            atomic_errors = atomic_total == 0;
        }
    }

    uint64_t digest = 0, work_items = 0;
    CUDA_OK(cudaMemcpy(&digest, d_digest, sizeof(digest),
                       cudaMemcpyDeviceToHost));
    CUDA_OK(cudaMemcpy(&work_items, d_work_items, sizeof(work_items),
                       cudaMemcpyDeviceToHost));
    int coverage = covered_sms(d_sm_hits, sm_count, hits);
    int expected_sms = opt.target_sm < 0 ? sm_count : 1;
    uint64_t total_errors = device_errors + compute_errors + atomic_errors;
    bool pass = total_errors == 0 && coverage >= expected_sms;
    double gib = static_cast<double>(work_items) * sizeof(uint32_t) /
                 (1024.0 * 1024.0 * 1024.0);
    print_result(opt.mode.c_str(), pass, rounds, total_errors, digest,
                 work_items, coverage, expected_sms, elapsed, gib);
    std::printf(
        "MIX_DETAIL flags=0x%x data_errors=%" PRIu64
        " compute_errors=%" PRIu64 " atomic_errors=%" PRIu64
        " atomic_ops=%" PRIu64 "\n",
        flags, device_errors, compute_errors, atomic_errors, atomic_total);
    ...
    if (d_output)
        CUDA_OK(cudaFree(d_output));
    if (d_vram)
        CUDA_OK(cudaFree(d_vram));
    if (d_l2)
        CUDA_OK(cudaFree(d_l2));
    return pass;
}
...
void usage(const char *argv0) {
    std::fprintf(
        stderr,
        "usage: %s [--mode all|compute|l2|vram|atomic|mix-cl|mix-la|"
        "mix-cla|mix-lv|mix-clva] [--seconds N] "
...
Options parse_options(int argc, char **argv) {
    ...
    if (opt.mode != "all" && opt.mode != "compute" && opt.mode != "l2" &&
        ...
        mixed_flags_for_mode(opt.mode) == 0) {
